...
// server URL is configured, and on demand from the toolbar gear button.
struct ConnectionSheet: View {
    ...
    var body: some View {
        ...
        .frame(width: 460)
        .onAppear {
            urlString = vm.serverURLString.nilIfEmpty ?? persistedURL.nilIfEmpty ?? "http://localhost:8088"
        }
    }
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("Connect to LP-100A Server")
                    .font(.headline)
                Text("Enter the URL of your LP-100A WebSocket bridge.")
                    .font(.subheadline)
    ...
    private var fieldSection: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            TextField("http://host:8088", text: $urlString, onCommit: connect)
    ...
    private var hintSection: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Tip")
                .font(.caption.bold())
                .foregroundStyle(.secondary)
            Text("On the server host: `lp100a-server` listens on port 8088 by default. Use http://localhost:8088 if running on this Mac, or the Pi's hostname/IP if running over the LAN.")
                .font(.caption)
                .foregroundStyle(.secondary)
    ...
    private var testIcon: String { "wave.3.right" }
    private var testIconColor: Color { .secondary }
